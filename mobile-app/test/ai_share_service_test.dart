...
import 'package:lecto/core/constants/app_constants.dart';
import 'package:lecto/core/services/ai_share_service.dart';
...
void main() {
  group('buildPrompt', () {
    ...
    test('explains that several files are one lecture', () {
      final prompt = AiShareService.buildPrompt(title: 'Lecture');

      expect(prompt, contains('consecutive'));
      expect(prompt, contains('filename order'));
    });

    test('asks for a transcript by default and can be told not to', () {
      expect(
        AiShareService.buildPrompt(title: 'L'),
        contains(AiShareService.transcriptHeading),
      );
      expect(
        AiShareService.buildPrompt(title: 'L', askForTranscript: false),
        isNot(contains(AiShareService.transcriptHeading)),
      );
    });
  ...
  group('file limits', () {
    test('leaves a slot for the prompt', () {
      ...
      expect(
        ...
        AiShareService.maxFilesPerShare - 1,
      );
    });
